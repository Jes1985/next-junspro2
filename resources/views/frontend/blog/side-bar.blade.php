<div class="col-lg-4">
  <div class="blog-sidebar-premium">
    ...
    <div class="sidebar-widget-premium mb-4">
      <form action="{{ route('blog') }}" method="GET" class="blog-search-form-premium">
    ...
    @if (count($categories) > 0)
      <div class="sidebar-widget-premium">
        <h4 class="widget-title-premium">{{ __('Catégories') }}</h4>
        <ul class="categories-list-premium">
          <li>
            <a href="#" class="category-link-premium @if (empty(request()->input('category'))) active @endif" data-category_slug="">
              <span class="category-name">{{ __('Tout') }}</span>
              <span class="category-count">{{ $totalPost }}</span>
            </a>
          </li>

          @foreach ($categories as $category)
            <li>
              <a href="#" class="category-link-premium @if ($category->slug == request()->input('category')) active @endif" data-category_slug="{{ $category->slug }}">
