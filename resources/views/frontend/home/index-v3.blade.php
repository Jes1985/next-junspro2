...
@section('content')
      ...
      <div class="row justify-content-center align-items-center">
        <div class="col-xl-7 col-lg-8">
          <div class="banner-content mw-100 text-center" data-aos="fade-up">
            <h1 class="title mb-25">
              {{ __('Trouvez le freelance parfait pour votre projet') }}
            </h1>
            <p class="text">
              {{ __('Plateforme premium de mise en relation entre clients et freelances experts, avec abonnements hebdomadaires flexibles.') }}
            </p>
                ...
                <form action="{{ url('/explore') }}" method="GET">
                  <div class="input-inline">
                    <input class="form-control border-0"
                      placeholder="{{ __('Ex. création de site web, montage vidéo, gestion Ads...') }}" type="text"
                      name="search">
                    <button class="btn btn-lg btn-primary rounded-pill" type="submit"
                      aria-label="button">{{ __('Rechercher') }}</button>
                  </div>
                </form>
              </div>
              @if (!empty($BasicExtends) && !is_null($BasicExtends->popular_tags))
                <div class="banner-tags mt-15">
                  <span class="color-dark">{{ __('Recherches fréquentes :') }}</span>
                  @php
                    $tags = explode(',', $BasicExtends->popular_tags);
                  @endphp
                  @foreach ($tags as $tag)
                    <a href="{{ url('/explore') . '?search=' . urlencode($tag) }}">{{ $tag }}</a>
                    @if (!$loop->last)
                      ,
                    @endif
                  @endforeach
        ...
        <div class="col-xl-5 d-none d-lg-block">
          <div class="banner-img" data-aos="fade-up">
            @if (!empty($heroImg))
              <img class="lazyload blur-up" data-src="{{ asset('assets/img/' . $heroImg) }}" alt="Junspro Hero">
            @endif
          </div>
        </div>
      </div>
        ...
        <div class="col-md-3 col-6" data-aos="fade-up">
          <div class="card border-0 h-100">
            <div class="card-body text-center">
              <span class="d-inline-flex justify-content-center align-items-center rounded-circle bg-primary-soft mb-3"
                style="width: 44px; height: 44px;">
                <i class="fas fa-lock text-primary"></i>
              </span>
              <h6 class="mb-1">{{ __('Paiements sécurisés') }}</h6>
              <p class="mb-0 small text-muted">{{ __('Transactions protégées et gérées par la plateforme.') }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="col-md-3 col-6" data-aos="fade-up" data-aos-delay="100">
          <div class="card border-0 h-100">
            <div class="card-body text-center">
              ...
              <h6 class="mb-1">{{ __('Livraisons avec rapport après chaque heure') }}</h6>
              <p class="mb-0 small text-muted">
                {{ __('50 min de travail + 10 min de rapport détaillé sur l’avancement.') }}</p>
            </div>
          </div>
        </div>
        <div class="col-md-3 col-6" data-aos="fade-up" data-aos-delay="150">
          <div class="card border-0 h-100">
            <div class="card-body text-center">
              ...
              <h6 class="mb-1">{{ __('Possibilité de changer de freelance') }}</h6>
              <p class="mb-0 small text-muted">
                {{ __('En cas de problème, transférez votre projet vers un autre freelance.') }}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  @php
    $ctaBgImg = 'newsletter-bg-3.jpg';
    $ctaSectionInfo->image = 'newsletter-3.png';
  @endphp
