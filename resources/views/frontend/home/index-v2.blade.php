...
@section('content')
                ...
                <div class="col-lg-7">
                    <div class="fluid-left">
                        <div class="banner-content" data-aos="fade-up">
                            ...
                            @else
                                <h1>{{ 'The Easiest Way to Find & Hire Skills Talent for Projects' }}</h1>
                            @endif
                                    ...
                                    <form action="{{ route('services') }}" method="GET">
                                        <div class="input-inline">
                                            <input class="form-control bg-white"
                                                placeholder="{{ __('e. g. Mobile application') }}" type="text"
                                                name="keyword">
                                            <button class="btn btn-lg btn-primary" type="submit"
                                                aria-label="button">{{ __('Search') }}</button>
                                        </div>
                                    </form>
                                </div>
                                @if (!empty($BasicExtends))
                                    @if (!is_null($BasicExtends->popular_tags))
                                        <div class="banner-tags mt-15">
                                            ...
                                            @foreach ($tags as $tag)
                                                <a
                                                    href="{{ route('services', ['keyword' => $tag]) }}">{{ $tag }}</a>
                                                @if (!$loop->last)
                                                    ,
                                                @endif
                                            @endforeach
                ...
                <div class="col-lg-5">
                    <div class="right-content">
                        ...
                        @if (!empty($heroVideoUrl))
                            <a href="{{ $heroVideoUrl }}" class="video-btn video-btn-white youtube-popup p-absolute"
                                style="margin-left: -50px;" title="{{ __('Play Video') }}">
                                    ...
                                    <div class="card border" style="height: 250px;">
                                        ...
                                        <div class="card_details p-25">
                                            ...
                                            <div class="card_action">
                                                <a href="{{ route('services', ['category' => $category->slug]) }}"
                                                    class="btn-text icon-end" title="{{ __('Show Service Gigs') }}"
                                                    target="_self">
                                        ...
                                        <ul class="nav nav-tabs" data-hover="fancyHover">
                                            @foreach ($featuredCategories as $featuredCategory)
                                                <li class="nav-item {{ $loop->first ? 'active' : '' }}">
                                                    <button
                                                        class="nav-link hover-effect btn-md radius-sm {{ $loop->first ? 'active' : '' }}"
                                                        data-bs-toggle="tab"
                                                        data-bs-target="#tab{{ $featuredCategory->slug }}"
                                                            ...
                                                            <div class="service_details col-xl-6">
                                                                <div class="authors mb-15">
                                                                    ...
                                                                    <a href="{{ route('service.update_wishlist', ['slug' => $serviceContent->slug]) }}"
                                                                        ...
                                                                        @auth('web')
                                                                            <i
                                                                                class="fas fa-heart {{ $wishlisted == true ? 'added-in-wishlist' : '' }}"></i>
                                                                        @endauth
                                                                        @guest('web')
                                                                ...
                                                                <div class="ratings size-md">
                                                                    <div class="rate bg-img"
                                                                        ...
                                                                        <div class="rating-icon bg-img"
                                                                            style="width: {{ $service->average_rating * 20 }}%"
                                                                            data-bg-img="{{ asset('assets/front/images/rate-star-md.png') }}">
                                                                        </div>
                                                                    </div>
                                                                    <span
                                                                        class="ratings-total">{{ $service->average_rating }}
                                                                        ({{ $reviewCount }})</span>
                                                                </div>
                                                                @php
                                                                    $position =
    ...
    @if ($secInfo->testimonials_section_status == 1 && count($testimonials) > 0)
    ...
    <!-- Blog-area start -->
    @if ($secInfo->blog_section_status == 1 && count($posts) > 0)
            ...
            <div class="container">
                <div class="section-title title-inline mb-50" data-aos="fade-up">
                    ...
                    <a href="{{ route('blog') }}" class="btn btn-lg btn-primary radius-sm"
                        title="{{ __('Read All Post') }}" target="_self">{{ __('Read All Post') }}</a>
                </div>
                <div class="row">
                                    ...
                                    <ul class="card_list list-unstyled mb-15">
                                        <li class="icon-start">
                                            <a href="#" target="_self"><i
                                                    class="fas fa-user"></i>{{ $post->author }}</a>
                                        </li>
                                        <li class="icon-start">
                                            <a href="{{ route('blog', ['category' => $post->categorySlug]) }}"
                                                target="_self" title="{{ $post->categoryName }}"><i
                                                    class="fas fa-th-large"></i>{{ $post->categoryName }}</a>
                                        </li>
                                        <li class="icon-start">
  ...
  @endif
    ...
    {{-- @if ($secInfo->cta_section_status == 1)
        <section class="newsletter-area newsletter-area_v1 pb-100" data-aos="fade-up">
            <div class="container">
                @php
                    $ctaBgImg = 'newsletter-bg-2.jpg';
                    $ctaSectionInfo->image = 'newsletter-2.png';
                @endphp
