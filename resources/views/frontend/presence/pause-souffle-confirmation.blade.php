...
@section('pageHeading')
  Votre Pause Souffle est réservée | {{ $websiteInfo->website_title }}
@endsection
  ...
  <div class="pause-souffle-confirmation-container">
    ...
    <div class="pause-souffle-confirmation-content">
      <div class="pause-souffle-confirmation-plan">
        <div class="pause-souffle-confirmation-plan-info">
          <h3>{{ $intake->plan_label }}</h3>
          <p>{{ ucfirst($intake->situation) }} • {{ ucfirst(implode(', ', $intake->protect ?? [])) }}</p>
        </div>
        <div class="pause-souffle-confirmation-status {{ $intake->status === 'paid' ? 'paid' : '' }}">
    ...
    <div class="pause-souffle-confirmation-next">
      ...
      <div class="pause-souffle-confirmation-actions">
        <a href="{{ route('services.corporate') }}?domain=pause-souffle" class="pause-souffle-confirmation-btn">
          Choisir un accompagnant
        </a>
        <a href="{{ route('services.corporate') }}?domain=pause-souffle&recommended=1" class="pause-souffle-confirmation-btn pause-souffle-confirmation-btn-secondary">
