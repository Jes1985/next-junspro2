  ...
  <div class="create-service-page">
    <div class="create-service-container">
      ...
      <div class="page-header-section">
        <h1>{{ __('Créer un nouveau service') }}</h1>
        <p>{{ __('Remplissez le formulaire ci-dessous pour créer votre Rituel et commencer à recevoir des demandes.') }}</p>
      ...
      <!-- Formulaire -->
      <form id="serviceForm" action="{{ route('freelance.services.store') }}" method="POST" enctype="multipart/form-data">
        @csrf

        <!-- Informations de base -->
        <div class="form-section">
          <h2 class="form-section-title">{{ __('Informations de base') }}</h2>

          <div class="form-group">
            <label for="title" class="required">{{ __('Titre du service') }}</label>
            <input type="text" id="title" name="title" class="form-control @error('title') is-invalid @enderror" 
                   value="{{ old('title') }}" placeholder="{{ __('Ex: Développement web personnalisé') }}" required>
            @error('title')
            ...
            <div class="description-editor-wrapper">
              ...
              <div class="editor-container">
                ...
                <div class="editor-content" id="editorContent" contenteditable="true" 
                     placeholder="{{ __('Décrivez votre Rituel en détail (minimum 30 caractères)...') }}"></div>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label for="category" class="required">{{ __('Catégorie') }}</label>
            ...
            <div class="col-md-6">
              <div class="form-group">
                <label for="price" class="required">{{ __('Prix (€)') }}</label>
        ...
        <!-- Images -->
        <div class="form-section">
          <h2 class="form-section-title">{{ __('Images') }}</h2>

          <div class="form-group">
            <label for="thumbnail_image" class="required">{{ __('Image miniature') }}</label>
            <input type="file" id="thumbnail_image" name="thumbnail_image" 
                   class="form-control @error('thumbnail_image') is-invalid @enderror" accept="image/*" required>
            <small class="text-muted">{{ __('Taille recommandée : 330 x 255 px. Taille max : 2 Mo') }}</small>
        ...
        <div class="text-center mt-4">
          ...
          <a href="{{ route('freelance.dashboard', ['tab' => 'services']) }}" class="btn btn-secondary ml-3" style="padding: 0.75rem 2rem; border-radius: 8px; text-decoration: none; display: inline-block; background: #f3f4f6; color: #374151;">
            {{ __('Annuler') }}
          </a>
        </div>
      </form>
    ...
    (function() {
      ...
      document.getElementById('serviceForm').addEventListener('submit', function(e) {
        ...
        if (content.trim().length < 30) {
          e.preventDefault();
          alert('{{ __('La description doit contenir au moins 30 caractères.') }}');
          editorContent.focus();
          return false;
        }
        ...
        if (emojiCount > 6) {
          e.preventDefault();
          alert('{{ __('Vous avez utilisé trop d\'emojis (maximum 6 autorisés).') }}');
          editorContent.focus();
          return false;
        }
