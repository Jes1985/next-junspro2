...
@section('content')
  <div class="onboarding-page">
    <div class="onboarding-container">
      ...
      <div class="onboarding-content">
        ...
        <form action="{{ route('freelance.onboarding.step3.store') }}" method="POST" enctype="multipart/form-data" id="certificationsForm">
          @csrf
          ...
          <div id="certificationsContainer" class="certifications-container">
            ...
            @foreach($certifications as $index => $cert)
              <div class="certification-item" data-cert-index="{{ $index }}">
                <div class="certification-item-header">
                  <span class="certification-item-title">Certification {{ $index + 1 }}</span>
                  @if($index > 0)
                ...
                <div class="form-group">
                  ...
                  <select name="certifications[{{ $index }}][subject]" class="form-select @error('certifications.' . $index . '.subject') form-input-error @enderror" required>
                    <option value="">Sélectionner...</option>
                    @if($freelancerProfile->skills)
                      @foreach($freelancerProfile->skills as $skill)
                        <option value="{{ $skill }}" {{ ($cert['subject'] ?? '') === $skill ? 'selected' : '' }}>{{ $skill }}</option>
                      @endforeach
                    @endif
                    <option value="Développement Web" {{ ($cert['subject'] ?? '') === 'Développement Web' ? 'selected' : '' }}>Développement Web</option>
                    <option value="Design Graphique" {{ ($cert['subject'] ?? '') === 'Design Graphique' ? 'selected' : '' }}>Design Graphique</option>
                    <option value="Marketing Digital" {{ ($cert['subject'] ?? '') === 'Marketing Digital' ? 'selected' : '' }}>Marketing Digital</option>
                    <option value="Autre" {{ ($cert['subject'] ?? '') === 'Autre' ? 'selected' : '' }}>Autre</option>
                  </select>
                  @error('certifications.' . $index . '.subject')
                ...
                <div class="form-group">
                  ...
                  <input 
                    ...
                    class="form-input @error('certifications.' . $index . '.name') form-input-error @enderror"
                    value="{{ $cert['name'] ?? '' }}"
                ...
                <div class="form-group">
                  ...
                  <textarea 
                    ...
                    class="form-textarea @error('certifications.' . $index . '.description') form-input-error @enderror"
                    placeholder="Décrivez votre certification, les compétences acquises, l'organisme délivrant..."
                    required
                  >{{ $cert['description'] ?? '' }}</textarea>
                  @error('certifications.' . $index . '.description')
                    <span class="form-error">{{ $message }}</span>
                  @enderror
                </div>
              </div>
          ...
          <div class="onboarding-actions">
            <a href="{{ route('freelance.onboarding.step2') }}" class="btn-back">
              ← Retour
            </a>
  ...
  <script>
    let certificationIndex = {{ count($certifications ?? [['subject' => '', 'name' => '', 'description' => '']]) }};
