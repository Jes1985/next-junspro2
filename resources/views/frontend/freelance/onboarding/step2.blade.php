...
@section('content')
  <div class="onboarding-page">
    <div class="onboarding-container">
      ...
      <div class="onboarding-content">
        ...
        <form action="{{ route('freelance.onboarding.step2.store') }}" method="POST" enctype="multipart/form-data" id="photoForm">
          @csrf
              ...
              <div class="profile-preview-card" style="flex-direction: column; align-items: flex-start; gap: 1rem;">
                <div style="display: flex; align-items: center; gap: 1rem; width: 100%;">
                  <div class="profile-preview-avatar" id="profileAvatar" style="width: 80px; height: 80px; font-size: 2rem;">
                    @if($user->image)
                      <img src="{{ asset('assets/img/users/' . $user->image) }}" alt="{{ $user->first_name ?? 'Freelance' }}">
                    @else
                      {{ strtoupper(substr($user->first_name ?? 'F', 0, 1)) }}{{ strtoupper(substr($user->last_name ?? 'L', 0, 1)) }}
                    @endif
                  </div>
                  <div class="profile-preview-info" style="flex: 1;">
                    <div class="profile-preview-name">{{ $user->first_name ?? 'Prénom' }} {{ $user->last_name ?? 'Nom' }}</div>
                    @if($freelancerProfile->skills && count($freelancerProfile->skills) > 0)
                      <div class="profile-preview-details" style="margin-top: 0.5rem;">
                        Domaine de compétences: {{ $freelancerProfile->skills[0] }}
                      </div>
                    @endif
                  </div>
                </div>
                @if($freelancerProfile->languages && count($freelancerProfile->languages) > 0)
                  <div style="width: 100%; padding-top: 1rem; border-top: 1px solid #e5e7eb;">
                    <div class="profile-preview-details">
                      <strong>Parle:</strong> 
                      @foreach($freelancerProfile->languages as $index => $lang)
                        {{ $lang['language'] ?? '' }} 
                        @if(isset($lang['level']))
                          ({{ $lang['level'] === 'native' ? 'Natif' : ($lang['level'] === 'fluent' ? 'Courant' : ($lang['level'] === 'intermediate' ? 'Intermédiaire' : 'Débutant')) }})
                        @endif
                        @if($index < count($freelancerProfile->languages) - 1), @endif
                      @endforeach
                    </div>
                  </div>
                @endif
          ...
          <div class="onboarding-actions">
            <a href="{{ route('freelance.onboarding.step1') }}" class="btn-back">
              ← Retour
            </a>
            <button type="submit" class="btn-continue" id="submitBtn" {{ $user->image ? '' : 'disabled' }}>
  ...
  <script>
    ...
    photoForm.addEventListener('submit', function(e) {
      ...
      if (!photoInput.files.length && !{{ $user->image ? 'true' : 'false' }}) {
        e.preventDefault();
