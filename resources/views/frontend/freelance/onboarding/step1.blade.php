...
@section('content')
  <div class="onboarding-page">
    <div class="onboarding-container">
      @include('frontend.freelance.onboarding.partials.premium-stepper', ['routeStep' => 1])
      <!-- Barre de progression -->
        ...
        <form action="{{ route('freelance.onboarding.step1.store') }}" method="POST" class="onboarding-form" enctype="multipart/form-data">
          ...
          <div class="form-group">
            <div class="onboarding-scope-col">
              ...
              <input type="hidden" name="intervention_mode" id="scopeModeInput" value="{{ $selectedMode ?? 'online' }}">
            </div>
          ...
          <div class="form-group">
            ...
            <div
              ...
              data-universes='@json($scopeUniverses)'
              data-domains='@json($scopeDomainsByUniverse)'
              data-countries='@json($scopeCountries)'
              data-cities='@json($scopeCitiesByCountry)'
              data-selected-universes='@json(array_values((array)$selectedUniverses))'
              data-selected-domains='@json(array_values((array)$selectedDomains))'
              data-specializations='@json($specializationsByDomain ?? [])'
              data-selected-specialization-main="{{ is_string($selectedSpecializationMain) ? $selectedSpecializationMain : '' }}"
              data-selected-specialization-additional='@json(array_values((array)$selectedSpecializationAdditional))'
            ...
            @error('domains')
              <span class="form-error">{{ $message }}</span>
            @enderror
            @error('intervention_mode')
              <span class="form-error">{{ $message }}</span>
            @enderror
            @error('onsite_country')
              <span class="form-error">{{ $message }}</span>
            @enderror
            @error('onsite_city')
              <span class="form-error">{{ $message }}</span>
            @enderror
            @error('specialization_main')
              <span class="form-error">{{ $message }}</span>
            @enderror
            @error('specialization_additional')
              <span class="form-error">{{ $message }}</span>
            @enderror
          </div>
          ...
          <div class="form-group">
            ...
            <div class="onboarding-scope-premium onboarding-mirror-filters" id="onboardingUniverseMirrorRoot">
              ...
              <div id="matchingUniversePanels" style="display:flex; flex-direction:column; gap:.65rem;">
                @foreach(($scopeUniverses ?? []) as $uSlug => $uLabel)
                  @php $m = is_array($matchingFilters[$uSlug] ?? null) ? $matchingFilters[$uSlug] : []; @endphp
                  @php $uf = (array)($onboardingUniverseFilters[$uSlug] ?? []); @endphp
                  <details class="onboarding-matching-panel" data-universe-panel="{{ $uSlug }}" data-onboarding-filters="{{ $uSlug }}" data-matching-payload='@json($m)' style="display:none; border:1px solid #e5e7eb; border-radius:12px; background:#fff; padding:.7rem;">
                    <summary style="cursor:pointer; font-weight:700; color:#111827;">{{ $uLabel }}</summary>
                    <div class="onboarding-mirror-universe-inner" style="margin-top:.7rem;">
                      @if($uSlug === 'projects')
                        @include('frontend.freelance.onboarding.partials.matching.projects', ['m' => $m, 'uf' => $uf])
                      @elseif($uSlug === 'lessons')
                        <div class="filter-group filter-group-rituel lessons-engagement-block" data-onboarding-lessons-rituel>
                          ...
                          <div class="engagement-select-wrapper">
                            <select name="price_range" class="filter-select budget-filter" data-lessons-budget>
                              ...
                              <option value="0-1000" {{ ($m['price_range'] ?? '') == '0-1000' ? 'selected' : '' }}>0 – 1 000 € — Budget formation — Exploratoire</option>
                              <option value="1000-2500" {{ ($m['price_range'] ?? '') == '1000-2500' ? 'selected' : '' }}>1 000 – 2 500 € — Budget formation — Standard</option>
                              <option value="2500-5000" {{ ($m['price_range'] ?? '') == '2500-5000' ? 'selected' : '' }}>2 500 – 5 000 € — Budget formation — Intensif</option>
                              <option value="5000-10000" {{ ($m['price_range'] ?? '') == '5000-10000' ? 'selected' : '' }}>5 000 – 10 000 € — Budget formation — Avancé</option>
                              <option value="10000-20000" {{ ($m['price_range'] ?? '') == '10000-20000' ? 'selected' : '' }}>10 000 – 20 000 € — Budget formation — Partenariat</option>
                              <option value="20000-60000" {{ ($m['price_range'] ?? '') == '20000-60000' ? 'selected' : '' }}>20 000 – 60 000 € — Budget formation — Long terme</option>
                              <option value="60000+" {{ ($m['price_range'] ?? '') == '60000+' ? 'selected' : '' }}>60 000 € et + — Budget formation — Stratégique étendu</option>
                            </select>
                  </div>
                            ...
                            <div class="filter-group besoin-tarif-rituel">
                              ...
                              <div class="rituel-price-filter-wrapper">
                                ...
                                <div class="rituel-price-range-display">
                                  <input type="number" name="price_min" class="rituel-price-input" data-lessons-price-min min="10" max="299" value="{{ $m['price_min'] ?? 10 }}" step="1">
                                  <span class="rituel-price-separator">-</span>
                                  <input type="number" name="price_max" class="rituel-price-input" data-lessons-price-max min="10" max="299" value="{{ $m['price_max'] ?? 50 }}" step="1">
                      ...
                      @elseif($uSlug === 'at-home' || $uSlug === 'wellnesslive')
                        <div class="besoin-rituel-row" data-onboarding-rituel="{{ $uSlug }}">
                          <div class="filter-group besoin-tarif-rituel">
                            <label class="filter-label"><i class="fas fa-coins me-2"></i>Tarif du rituel / h</label>
                            <div class="rituel-price-filter-wrapper">
                              <div class="rituel-price-slider" data-rituel-slider></div>
                              <div class="rituel-price-range-display">
                                <input type="number" name="price_min" class="rituel-price-input" data-rituel-price-min min="10" max="100" value="{{ $m['price_min'] ?? 10 }}" step="1">
                                <span class="rituel-price-separator">-</span>
                                <input type="number" name="price_max" class="rituel-price-input" data-rituel-price-max min="10" max="100" value="{{ $m['price_max'] ?? 50 }}" step="1">
                                <span class="rituel-price-unit">€</span>
                              </div>
                      ...
                      @if($uSlug === 'homeswap')
                        @include('frontend.freelance.onboarding.partials.matching.homeswap', ['m' => $m, 'uf' => $uf])
                      @elseif($uSlug === 'corporate' || $uSlug === 'presence')
                        @include('frontend.freelance.onboarding.partials.matching.presence', ['m' => $m, 'uf' => $uf])
                      @elseif($uSlug === 'lessons' || $uSlug === 'at-home' || $uSlug === 'wellnesslive')
                        @include('frontend.freelance.onboarding.partials.matching.universal', ['m' => $m, 'uf' => $uf, 'uSlug' => $uSlug])
                      @endif
