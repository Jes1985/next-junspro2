      ...
      if (cancelEmailBtn && editEmailForm) {
        cancelEmailBtn.addEventListener('click', function() {
          ...
          if (newEmailInput && emailUpdateForm) {
            newEmailInput.value = '{{ $user->email_address ?? '' }}';
  ...
  <div class="complete-page">
    <div class="complete-container">
      ...
      <div style="background: #F9FAFB; border: 1px solid #E5E7EB; border-radius: 12px; padding: 1.5rem; margin-bottom: 2rem;">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1rem;">
          <div>
            ...
            <p style="font-size: 0.875rem; color: #6B7280; margin: 0;">{{ $user->email_address ?? '' }}</p>
          </div>
        ...
        <div id="editEmailForm" style="display: none; padding-top: 1rem; border-top: 1px solid #E5E7EB;">
          <form action="{{ route('freelance.onboarding.update_email') }}" method="POST" id="emailUpdateForm">
            @csrf
            <div style="margin-bottom: 1rem;">
              ...
              <input 
                ...
                name="email" 
                id="newEmailInput"
                value="{{ old('email', $user->email_address ?? '') }}"
                required
                style="width: 100%; padding: 0.75rem; border: 1px solid #D1D5DB; border-radius: 8px; font-size: 0.875rem;"
                placeholder="nouvelle@email.com"
              >
              @if($errors->has('email'))
                <p style="color: #EF4444; font-size: 0.875rem; margin-top: 0.5rem;">{{ $errors->first('email') }}</p>
              @endif
            </div>
      ...
      <div class="complete-actions">
        <a href="{{ route('index') }}" class="btn-understand">
