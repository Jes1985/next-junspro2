...
@section('style')
  ...
  <style>
    ...
    /* ──── Widget "Ma semaine" / Calendrier client ──── */
    ...
    .cwc-day--has-sessions:not(.cwc-day--today) {
      border-color: #7C3AED;
      background: #F5F3FF;
    }

    .cwc-day--today .cwc-day-name,
    .cwc-day--today .cwc-day-num { color: white; }
    ...
    .cwc-day--today .cwc-dot { background: rgba(255,255,255,0.85); }
    .cwc-empty { opacity: 0; pointer-events: none; }
    ...
    .cwc-fl-info { flex: 1; min-width: 0; }
    ...
    .cwc-freelancer-card:hover .cwc-fl-action {
      background: #4F46E5;
      color: white;
    }

    @media (max-width: 480px) {
      .client-week-calendar { gap: 4px; }
      .cwc-day { padding: 8px 2px; }
      .cwc-day-num { font-size: 14px; }
    }
  </style>
@endsection
...
@section('content')
  <div class="client-dashboard-container">
    ...
    {{-- 1.5) Widget "Ma semaine" — calendrier & programmation rapide --}}
    @php
      $calWeekStart = \Carbon\Carbon::now()->startOfWeek(\Carbon\Carbon::MONDAY);
      $calWeekEnd   = $calWeekStart->copy()->endOfWeek(\Carbon\Carbon::SUNDAY);
      $calWeekDays  = [];
      $frDays = ['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'];
      for ($di = 0; $di < 7; $di++) {
        $calDay = $calWeekStart->copy()->addDays($di);
        $calDaySessions = $upcomingSessions->filter(fn($s) =>
          \Carbon\Carbon::parse($s->start_at)->isSameDay($calDay)
        );
        $calWeekDays[] = ['date' => $calDay, 'sessions' => $calDaySessions, 'label' => $frDays[$di]];
      }
      $activeSubsWithPending = $subscriptions->filter(fn($s) =>
        $s->status === 'active' && ($s->hours_remaining ?? 0) >= 0.5
      );
    @endphp

    @if($subscriptions->count() > 0)
    <div class="client-calendar-widget">
      <div class="section-header" style="margin-bottom: 0;">
        <h2 class="section-title" style="display:flex; align-items:center; gap:8px;">
          <i class="fas fa-calendar-week" style="color:#7C3AED; font-size:18px;"></i>
          Ma semaine
        </h2>
        <span style="font-size:12px; color:#9CA3AF; font-weight:500;">
          {{ $calWeekStart->translatedFormat('d M') }} – {{ $calWeekEnd->translatedFormat('d M Y') }}
        </span>
      </div>

      {{-- Grille des 7 jours --}}
      <div class="client-week-calendar">
        @foreach($calWeekDays as $dayData)
          @php
            $isToday = $dayData['date']->isToday();
            $hasSessions = $dayData['sessions']->count() > 0;
          @endphp
          <div class="cwc-day {{ $isToday ? 'cwc-day--today' : '' }} {{ $hasSessions ? 'cwc-day--has-sessions' : '' }}">
            <div class="cwc-day-name">{{ $dayData['label'] }}</div>
            <div class="cwc-day-num">{{ $dayData['date']->format('d') }}</div>
            @if($hasSessions)
              <div class="cwc-session-dots">
                @foreach($dayData['sessions']->take(3) as $sess)
                  <div class="cwc-dot" title="{{ \Carbon\Carbon::parse($sess->start_at)->format('H:i') }}"></div>
                @endforeach
              </div>
            @else
              <div class="cwc-session-dots cwc-empty"><div class="cwc-dot"></div></div>
            @endif
          </div>
        @endforeach
      </div>

      {{-- Freelances actifs avec Rituels à programmer --}}
      @if($activeSubsWithPending->count() > 0)
        <div class="cwc-freelancers-row">
          @foreach($activeSubsWithPending->take(3) as $sub)
            @php
              $flUser = $sub->freelancer->user ?? null;
              $flName = $flUser ? trim(($flUser->first_name ?? '') . ' ' . ($flUser->last_name ?? '')) : 'Freelance';
              if(empty(trim($flName))) $flName = $flUser->name ?? 'Freelance';
              $pendingRituels = (int) floor($sub->hours_remaining ?? 0);
            @endphp
            <a href="{{ route('client.subscriptions.show', $sub->id) }}" class="cwc-freelancer-card">
              @if($flUser && $flUser->image)
                <img src="{{ asset('assets/img/users/'.$flUser->image) }}"
                     alt="{{ $flName }}"
                     class="cwc-fl-avatar"
                     onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
                <div class="cwc-fl-avatar cwc-fl-initials" style="display:none;">{{ strtoupper(substr($flName,0,1)) }}</div>
              @else
                <div class="cwc-fl-avatar cwc-fl-initials">{{ strtoupper(substr($flName,0,1)) }}</div>
              @endif
              <div class="cwc-fl-info">
                <div class="cwc-fl-name">{{ $flName }}</div>
                @if($pendingRituels > 0)
                  <div class="cwc-fl-pending">
                    <i class="fas fa-circle" style="font-size:7px; vertical-align:middle; margin-right:4px;"></i>
                    {{ $pendingRituels }} Rituel{{ $pendingRituels > 1 ? 's' : '' }} à programmer
                  </div>
                @else
                  <div class="cwc-fl-pending" style="color:#10B981;">
                    <i class="fas fa-check-circle" style="font-size:12px; margin-right:4px;"></i>
                    Abonnement actif
                  </div>
                @endif
              </div>
              <div class="cwc-fl-action">
      ...
      @elseif($upcomingSessions->count() == 0)
        <div style="text-align:center; padding:1.5rem; background:#F9FAFB; border-radius:12px; margin-top:1rem;">
          <p style="color:#6B7280; font-size:14px; margin:0 0 12px;">Aucun Rituel prévu cette semaine.</p>
          <a href="{{ route('explore') }}" class="btn-hero-primary" style="font-size:13px; padding:8px 20px; display:inline-flex; align-items:center; gap:6px; border-radius:20px;">
