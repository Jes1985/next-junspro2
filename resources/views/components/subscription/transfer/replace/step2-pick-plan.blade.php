<!-- Étape 2 : Choisissez votre Rituel avec [Nom] -->
<div 
  x-data="transferReplaceStep2()"
  x-show="isOpen"
  x-cloak
  ...
  >
    ...
    <div class="transfer-modal-header">
      ...
      <h2 class="transfer-modal-title">
        <div>{{ __('Choisissez votre Rituel avec') }}</div>
        <div><span x-text="selectedFreelance?.name"></span>.</div>
      </h2>
      
      <!-- Texte intro -->
      <div class="transfer-ritual-intro">
        <p>{{ __('Un Rituel est un rythme d\'accompagnement sur 4 semaines, ajustable à tout moment.') }}</p>
        <p>{{ __('Chaque Rituel inclut 50 minutes de focus + 10 minutes de feedback, pour avancer sereinement et durablement.') }}</p>
      </div>
      
      <!-- Solde restant qui sera utilisé -->
      <div x-show="creditAmount > 0" class="transfer-balance-box">
        <div class="transfer-balance-content">
          <span>{{ __('Le reste de votre solde avec') }} <span x-text="currentTutorName"></span> (<span x-text="formatPrice(creditAmount)"></span>) {{ __('sera utilisé pour régler l\'abonnement avec') }} <span x-text="selectedFreelance?.name"></span>.</span>
        </div>
    ...
    <div class="transfer-modal-body transfer-ritual-body">
      ...
      <div class="transfer-ritual-selected-value">
        <span x-text="selectedValue"></span> <span class="transfer-ritual-unit">{{ __('h') }}</span>
      </div>
      ...
      <div class="transfer-ritual-slider-wrapper">
        <input 
          ...
          :aria-label="'{{ __('Rituels par semaine') }}'"
          :aria-valuemin="0"
      ...
      >
        <span x-text="showExtended ? '{{ __('Voir moins') }}' : '{{ __('Voir plus') }}'"></span>
      ...
      <div class="transfer-ritual-tiers">
        <div class="transfer-ritual-tiers-row">
          ...
          >
            ...
            <div class="transfer-ritual-tier-title">{{ __('Découverte') }}</div>
            <div class="transfer-ritual-tier-subtitle">{{ __('Entrer dans l\'expérience') }}</div>
          </button>
          ...
          >
            ...
            <div class="transfer-ritual-tier-title">{{ __('Continuité') }}</div>
            <div class="transfer-ritual-tier-subtitle">{{ __('Installer une habitude') }}</div>
          </button>
          ...
          >
            ...
            <div class="transfer-ritual-tier-badge">{{ __('Populaire') }}</div>
            <div class="transfer-ritual-tier-title">{{ __('Équilibre') }}</div>
            <div class="transfer-ritual-tier-subtitle">{{ __('Le rythme naturel') }}</div>
          </button>
        </div>
        ...
        <div class="transfer-ritual-tiers-row">
          ...
          >
            ...
            <div class="transfer-ritual-tier-title">{{ __('Engagement') }}</div>
            <div class="transfer-ritual-tier-subtitle">{{ __('Avancer concrètement') }}</div>
          </button>
      ...
      <div class="transfer-ritual-summary">
        <div class="transfer-ritual-summary-item">
          <span>{{ __('Vous choisissez :') }}</span>
          <strong x-text="selectedValue + ' ' + (selectedValue === 1 ? '{{ __('Rituel') }}' : '{{ __('Rituels') }}') + ' / semaine'"></strong>
        </div>
        <div class="transfer-ritual-summary-item">
          <span>{{ __('Soit :') }}</span>
          <strong x-text="(selectedValue * 4) + ' {{ __('Rituels') }} / x 4 semaines'"></strong>
        </div>
      </div>
      ...
      <div class="transfer-ritual-price">
        <div class="transfer-ritual-price-total">
          {{ __('Total :') }} <span x-text="formatPrice(calculatedPrice)"></span> {{ __('toutes les 4 semaines') }}
        </div>
        <div class="transfer-ritual-price-note">
          {{ __('Besoin ponctuel ? Vous pourrez ajouter des Rituels en plus, à tout moment, après l\'abonnement.') }}
        </div>
      </div>
    </div>
...
<style>
  ...
  /* Réduction de la hauteur du header de 2 cm (≈75px) - Ajusté pour agrandir de 2 cm */
  ...
  .transfer-ritual-container .transfer-balance-box {
    margin-top: 12px;
    padding: 14px 16px;
  }

  .transfer-ritual-container .transfer-balance-icon {
    width: 36px;
    height: 36px;
    font-size: 16px;
  }
...
<script>
  function transferReplaceStep2() {
    return {
      ...
      init() {
        window.addEventListener('openTransferReplaceStep2', (e) => {
          this.open(e.detail);
        });
      },
      ...
      open(payload) {
        ...
        this.pricePerSession = this.selectedFreelance?.price_per_session || 0;
      ...
      updateSelectedValue(value) {
        ...
        // Snapper sur la valeur la plus proche dans visibleValues
        const closest = this.visibleValues.reduce((prev, curr) => {
      ...
      get calculatedPrice() {
        ...
        return this.pricePerSession * totalSessions;
      },
