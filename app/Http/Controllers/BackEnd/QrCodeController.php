...
class QrCodeController extends Controller
{
  ...
  public function generate()
  {
    ...
    if (is_null($bs->qr_image) || !file_exists(public_path('./assets/img/qr-codes/' . $bs->qr_image))) {
      ...
      $this->qrCodeGenerator->generate(url('/'), $qrLocation, [
        'size' => 250,
        'margin' => 0,
    ...
    return view('backend.qr-code.generate', compact('bs'));
  }
  ...
  public function regenerate(Request $request)
  {
    ...
    $this->qrCodeGenerator->generate($request->url, $qrLocation, [
  ...
  public function savedCodes(Request $request)
  {
    $seller = null;
    ...
    $qrcodes = QRCodeModel::query()
      ...
      ->orderByDesc('id')->get();
    $sellers = Seller::where([['status', 1], ['id', '!=', 0]])->get();

    return view('backend.qr-code.saved-codes', compact('qrcodes', 'sellers'));
  }

  public function deleteQrCode($id)
  {
    $qrcode = QRCodeModel::query()->find($id);
