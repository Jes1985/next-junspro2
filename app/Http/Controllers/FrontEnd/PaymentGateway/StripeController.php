...
class StripeController extends Controller
{
  public function index(Request $request, $data, $paymentFor)
  {
    ...
    try {
      $stripeClient = new StripeClient(Config::get('services.stripe.secret'));

      try {
        $charge = $stripeClient->charges->create([
          'source' =>  $request->stripeToken,
          'currency' => 'USD',
          'amount'   => $this->formatAmountForStripe($stripeTotal),
  ...
  protected function formatAmountForStripe(float $amount): int
  {
